...
class X931State:
    def __init__(self, seed):
        ...
        self.V   = bytearray([int(i) for i in [*seed[:16]]])    #create bytearray of length 16 from repeating seed
        self.K   = bytearray(reversed(self.V))                  #key is reversed vector
        self.dt  = 0                                            #date time
        self.aes = AES.new(self.K, AES.MODE_ECB)                #aes encryptor

    def next(self):
        I        = self.aes.encrypt(self.dt.to_bytes(16, 'big'))    #generate I from date time (big endian)
        R        = self.aes.encrypt(arrayXor(I, self.V))            #generate result
        self.V   = self.aes.encrypt(arrayXor(I, R))                 #update vector
        self.dt += 1                                                #update date time
        return R


def generate_bytes(seed: int, length: int) -> bytes:
    generator = X931State(seed)
    result    = bytearray()

    #while instead of a for loop, as length might not be divisable by 16 (number of bytes we get from generator)
    while length > len(result):
        result.extend(generator.next()[:length - len(result)])   #copy only the remaining number of wanted bytes
